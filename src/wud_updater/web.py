...
from .db import DatabaseError, connect_db, init_db
...
def api_runs(request: Request) -> list[RunSummary]:
    settings = _settings(request)
    with _connect_initialized(settings) as conn:
        rows = conn.execute(
            """
            SELECT *
            FROM update_runs
...
def _database_ready(settings: WebSettings) -> tuple[bool, str]:
    ...
    try:
        conn = connect_db(settings.config.db_path)
        init_db(conn)
        return True, ""
...
def _connect_initialized(settings: WebSettings) -> sqlite3.Connection:
    conn: sqlite3.Connection | None = None
    try:
        conn = connect_db(settings.config.db_path)
        init_db(conn)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"could not initialize database: {exc}",
        ) from exc
    return conn
