...
REPOSITORY = "magrhino/WUD-Updater"
...
def load_ascii_art() -> str:
    checkout_art = Path(__file__).resolve().parents[2] / "local-dev" / (
        "ascii-text-art.txt"
    )
    try:
        if checkout_art.is_file():
            return checkout_art.read_text(encoding="utf-8")
    except OSError:
        pass
    try:
        return files("wud_updater").joinpath("ascii-text-art.txt").read_text(
            encoding="utf-8"
        )
...
def fetch_latest_release_tag(*, timeout: float = DEFAULT_RELEASE_TIMEOUT) -> str | None:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": f"wud-updater/{__version__}",
        },
    )
    try:
...
def _banner_enabled(env: dict[str, str], stream: TextIO) -> bool:
    value = _normalized_env_value(env.get("WUD_UPDATER_BANNER"))
...
def _release_check_enabled(env: dict[str, str]) -> bool:
    value = _normalized_env_value(env.get("WUD_UPDATER_RELEASE_CHECK"))
