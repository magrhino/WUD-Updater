"""Guided first-run configuration generation for WUD-Updater."""
...
def answers_from_namespace(
    ...
    *,
    ...
    if not raw_db_path and not non_interactive and profile == "host":
        raw_db_path = prompt.text("SQLite DB path", str(log_dir / "wud-updater.sqlite"))
    db_path = Path(raw_db_path or str(log_dir / "wud-updater.sqlite"))
...
def _env_content(answers: InitAnswers) -> str:
    if answers.profile == "host":
        ...
        title = "WUD-Updater host configuration generated by wud-updater init."
        return _shell_env_content(title, values)

    values = _container_env_values(answers)
    title = f"WUD-Updater {answers.profile} Compose environment generated by wud-updater init."
...
def _compose_override_content(answers: InitAnswers) -> str:
    ...
    data: dict[str, object] = {"services": {"wud-updater": {}}}
    service = data["services"]["wud-updater"]  # type: ignore[index]
    ...
    output = StringIO()
    output.write("# Generated by wud-updater init. Review before deploying.\n")
...
def _container_doctor_command(answers: InitAnswers) -> list[str]:
    ...
    command.extend(("run", "--rm", "wud-updater", "doctor"))
    return command
...
def _resolve_config_file(
    ...
    if profile in {"helper", "hardened"}:
        name = f"{profile}.env"
    return home / ".config" / "wud-updater" / name
...
def _resolve_compose_override(
    ...
    if profile not in {"helper", "hardened"}:
        return None
    return home / ".config" / "wud-updater" / f"docker-compose.{profile}.override.yml"
