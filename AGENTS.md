...
| Python backend validation | See `src/wud_updater/AGENTS.md`. |
...
- Python updater/config change: use `src/wud_updater/AGENTS.md`.
...
- WebUI frontend change: use `webui/AGENTS.md`; also use `src/wud_updater/AGENTS.md` when API contracts or auth assumptions change.
- Local WebUI browser check: avoid Codex in-app Browser and Chromium-backed Playwright on this setup because they have repeatedly crashed Codex/Chromium; prefer WebKit with `npm --prefix webui exec playwright -- test --browser=webkit`, or report browser verification skipped.
