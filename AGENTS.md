...
- Security workflow change: run `actionlint` when available, `git diff --check`, `tests/run-all.sh`, and the local `zizmor` command when installed; verify GHAS-backed CodeQL, Dependency Review, and SARIF uploads in the first GitHub run.
