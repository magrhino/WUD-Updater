...
| `WUD_API_BASE_URL` | `http://wud:3000` | Internal WUD API base URL used for best-effort WebUI metadata discovery. Auth-required or unavailable WUD is reported as degraded and does not change pending/apply behavior, which still uses `WUD_OUT_FILE`. Runtime discovery retries automatically on later WebUI requests after transient WUD API outages. |
